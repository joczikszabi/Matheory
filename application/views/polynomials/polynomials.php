<section id="border-orange" class="prime_factorization">
	<header class="section-name">
		<h2>Prime Factorization</h2>
	</header>
	<div class="description">
		<span>If R is an 
			<a class="btn btn-link highlight-blue helper-btn euclidean-ring-helper-btn" type="button" role="button" tabindex="0" data-trigger="focus" data-toggle="popover" data-html="true"
			data-content="<span class='popover-text euclidean-ring-helper-popover-text'>Let $$R$$ be an integral domain. An Euclidean function on $$R$$ is a function $$f$$ from $$R \backslash \{0\}$$ to the non-negative integers satisfying the following division-with-remainder property:<br /><br />- If $$a$$ and $$b$$ are in $$R$$ and $$b$$ is nonzero, then there are $$q$$ and $$r$$ in $$R$$ such that $$a = bq + r$$ and either $$r = 0$$ or $$f(r) \lt f(b)$$.<br />- For all nonzero $$a$$ and $$b$$ in $$R$$, $$f(a) \leq f(ab)$$.</span>">
			euclidean ring
			</a>, then every element in R is either a unit in R or can be written as the product of a finite number of 
			<a class="btn btn-link highlight-blue helper-btn prime-helper-btn" type="button" role="button" tabindex="0" data-trigger="focus" data-toggle="popover" data-html="true" data-placement="bottom"
			data-content="<span class='popover-text prime-popover-text'>A non-zero non-unit element $$p$$ in a commutative ring $$R$$ is called prime if, whenever $$p \mid bc$$ for some $$b$$ and $$c$$ in $$R$$, then $$p \mid b$$ or $$p \mid c$$.</span>">
			prime
			</a> elements in R. 
		</span>
	</div>
	<div class="example">
		...
		<div class="prime_factorization-example-text example-text">
			<span>$$84 = 2^2 \cdot 3 \cdot 7$$ where 
		...
		<?php 
			if (isset($factors)) {
				if ($factors != False) {
					...
					echo "<span class=\"expression\">$$".$_GET['input-prime_factorization']." = <span class=\"highlight-orange\">";
					foreach ($factors as $prime => $power) {
						if ($counter != $size) {
							if ($power != 1) {
								echo $prime."^".$power."\cdot";                
								$counter++;
							} else {
								echo $prime."\cdot";                
								$counter++;
...
<section id="border-blue" class="canonical-representation">
	<header class="section-name">
...
<section id="border-green" class="divisors">
	<header class="section-name">
		<h2>Divisors of a number</h2>
	</header>
	<div class="description">
		<span>If $$n = p_1^{\alpha_1} p_2^{\alpha_2} \cdots \ p_k^{\alpha_k}$$ &nbsp;,then $$d &#62; 0$$ divides $$n$$ if and only if $$d = p_1^{\beta_1} p_2^{\beta_2} \cdots \ p_k^{\beta_k}$$ &nbsp;where $$ 0 \leq \beta_i \leq \alpha_i, $$ $$i = 1,2, \cdots ,k$$. We get the two trivial divisors (1 and n itself) if $$\beta_i = 0$$ or $$\beta_i = \alpha_i$$ for all &nbsp;i. </span>
	</div>
	<div class="theorem">
		<div class="theorem-text">
			<span>The number of divisors of n is $$d(n) = (\alpha_1 + 1)(\alpha_2 + 1) \cdots (\alpha_k + 1)$$.&nbsp;&nbsp;<button class="btn btn-link divisors-proof-btn proof-btn" type="button">Why? </button></span>
		</div>
		<div class="divisors-proof proof">
			<span>Using the definition above, we can get all the divisors of $$n$$ if in the form $$d = p_1^{\alpha_1} p_2^{\alpha_2} \cdots \ p_k^{\alpha_k}$$ we loop through all the $$\beta_1, \beta_2, \cdots ,\beta_k$$ powers of $$p_i$$ independetly of each other where the possible values are $$ \beta_1 = 1, 2, \cdots ,\alpha_1$$, &nbsp;$$\beta_2 = 1, 2, \cdots ,\alpha_2$$, &nbsp;$$ \cdots \ $$ $$\beta_k = 1, 2, \cdots ,\alpha_k$$. In this case $$\beta_i$$ can have $$(\alpha_i + 1)$$ different kind of values. </span>
		</div>
	</div>
	<div class="section-ending test">
		<div class="test-header">
...
<section id="border-yellow" class="gcd">
	...
	<div class="theorem">
		<div class="theorem-text">
			<span>If $$a$$ and $$b$$ are both positive integers and $$a = p_1^{\alpha_1} p_2^{\alpha_2} \cdots p_k^{\alpha_k}$$, $$b = p_1^{\beta_1} p_2^{\beta_2} \cdots p_k^{\beta_k}$$ where $$\alpha_i, \beta_i \geq 0$$ then their greatest common divisor is $$d = p_1^{min(\alpha_1, \beta_1)} p_2^{min(\alpha_2, \beta_2)} \cdots p_k^{min(\alpha_k, \beta_k)}.$$</span>
...
<section id="border-lightblue" class="lcm">
	<header class="section-name">
		<h2>Least Common Multiple </h2>
	</header>
	<div class="description">
		<span>The least common multiple of two positive integers $$a$$ and $$b$$ is $$k$$ if $$a \mid k$$ and $$b \mid k$$ and for every $$c$$ such that $$a \mid c$$ and $$b \mid c$$, $$k \lt c$$.</span>
	</div>
	<div class="theorem">
		<div class="theorem-text">
			<span>If $$a$$ and $$b$$ are both positive integers and $$a = p_1^{\alpha_1} p_2^{\alpha_2} \cdots \ p_k^{\alpha_k}$$, $$b = p_1^{\beta_1} p_2^{\beta_2} \cdots \ p_k^{\beta_k}$$ where &nbsp; $$\alpha_i, \beta_i \gt 0$$, then &nbsp;their least common multiple is $$d = p_1^{max(\alpha_1, \beta_1)} p_2^{max(\alpha_2, \beta_2)} \cdots \ p_k^{max(\alpha_k, \beta_k)}.$$</span>
			<button class="btn btn-link lcm-proof-btn proof-btn" type="button">Why?</button>
		</div>
		<div class="lcm-proof proof">
			<span>A $$k$$ positive integer is a common multiple of $$a$$ and $$b$$ if and only if $$a \mid k$$ and $$b \mid k$$. Let $$k = p_1^{\gamma_1} p_2^{\gamma_2} \cdots \ p_k^{\gamma_k}$$. If $$k$$ is a common multiple of $$a$$ and $$b$$, then for every $$\gamma_i$$, $$\gamma_i \ge \alpha_i$$ and $$\gamma_i \ge \beta_i$$ where $$i = 1,2, \cdots ,k$$ which is equivalent to $$\gamma_i \ge max(\alpha_i, \beta_i)$$. The smallest number of these common multiples is the one where $$\gamma_i = max(\alpha_i, \beta_i).$$</span>
		</div>
	</div>
	<div class="section-ending test">
		<div class="test-header">
